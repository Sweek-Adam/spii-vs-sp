...
## 2. Voie rapide : le script d'initialisation
...
## 4. Procédure manuelle détaillée & dépannage

Cette partie détaille l'installation **pas à pas, à la main**. Tu n'en as pas
besoin si `initialiser.ps1` (voie rapide) a tout fait — garde-la sous la main
en cas de souci, ou si tu préfères tout faire toi-même.

### 4.1 Installer Python en version portable (WinPython, sans droits admin)
...
> déjà installé — tu peux sauter directement à la section 4.3 pour remplir tes
...
4. Dans le dossier décompressé, tu trouveras un fichier nommé
   **`WinPython Command Prompt.exe`**. C'est LUI qu'on utilisera pour lancer
   ...
   Tu dois voir `Python 3.14.x` (ou la version que tu as prise).
...
Tu n'as **pas** besoin d'installer `tomli` : WinPython est en Python 3.14, qui
a déjà le lecteur TOML intégré.

### 4.3 Préparer les fichiers de configuration
...
- **Erreur de module (`No module named ...`)** → la dépendance n'est pas
  installée dans CE WinPython (refais la section 4.2 depuis le WinPython Command
  Prompt).
- **`pip install pandas` échoue** avec `Unknown compiler` / `vswhere.exe` /
  ...
  ou 3.13 stable** (voir l'avertissement à la section 4.1).
